...
def test_load_config_uses_default_max_parallel_workers(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert config.queue_dir == tmp_path / "artifacts" / "queue"
    assert config.pool_result_timeout_seconds == 300
...
def test_load_config_accepts_worker_pool_backend(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("CMY_PROJECT_ROOT", str(tmp_path))
    monkeypatch.setenv("CMY_EXECUTION_BACKEND", "worker_pool")

    config = load_config()

    assert config.execution_backend == "worker_pool"
