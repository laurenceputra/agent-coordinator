...
class ManagerAgent:
    def __init__(self, config: RuntimeConfig) -> None:
        ...
        self._queue = FileTaskQueue(config.queue_dir)
    ...
    def _run_task_with_retries(self, task: TaskSpec) -> dict[str, Any]:
        ...
        for attempt in range(1, self._config.retry_limit + 1):
            ...
            if self._config.execution_backend == "worker_pool":
                result = self._execute_via_worker_pool(task, assignment, attempt)
            else:
    ...
    def _execute_via_worker_pool(self, task: TaskSpec, assignment: Assignment, attempt: int) -> WorkResult:
        self._queue.enqueue(
            task.task_id,
            {
                "task": {
                    ...
                    "parallelism_hint": task.parallelism_hint,
        ...
        deadline = time.monotonic() + self._config.pool_result_timeout_seconds
        while time.monotonic() < deadline:
            self._queue.requeue_expired(self._config.pool_lease_timeout_seconds, time.time())
            failed = self._queue.read_failed(task.task_id)
            if failed is not None:
                return WorkResult(
                    assignment=assignment,
                    output="",
                    checks_run=[],
                    exit_code=failed.get("exit_code", 1),
                    stderr=failed.get("stderr", "worker-pool-failed"),
                    duration_ms=int(failed.get("duration_ms", 0)),
                    worktree_path=str(failed.get("worktree_path", "")),
                )
            payload = self._queue.read_result(task.task_id)
            ...
            time.sleep(self._config.pool_poll_interval_seconds)
        return WorkResult(
            assignment=assignment,
            output="",
            checks_run=[],
            exit_code=1,
            stderr="worker-pool-timeout",
            duration_ms=int(self._config.pool_result_timeout_seconds * 1000),
            worktree_path="",
        )
