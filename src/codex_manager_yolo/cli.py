...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pool-worker", action="store_true", help="Run as worker-pool consumer")
    parser.add_argument("--pool-worker-type", choices=["codex-cli", "copilot-cli"])
    parser.add_argument("--pool-worker-id", default="worker")
    return parser
...
def main() -> None:
    ...
    if args.pool_worker:
        if not args.pool_worker_type:
            raise RuntimeError("--pool-worker-type is required when --pool-worker is set")
        raise SystemExit(run_pool_worker(config, args.pool_worker_type, args.pool_worker_id))
